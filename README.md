...
- 클래스나 인터페이스가 컴파일 되면 크래스 파일(.class) 생성
...
# 자바 플랫폼의 모듈화
> 자바 플랫폼
- 자바의 개발 환경(JDK)과 자바이 실행 환경(JRE)을 자칭 Java SE(자바 API) 포함
- 자바 API의 모든 클래스가 여러 개의 모듈로 재구성됨 
- 모듈 파일은 jdk의 jmods 디렉터리에 저장하여 배포. 현재는 OpenJDK에 따라 따름.
...
# 박싱과 언박싱
> 박싱: 기본 타입의 값을 WRapper 객체로 변환하는것
> 언박싱: Wrapper 객체에 들어있는 기본 타입의 값을 빼내는것. 박싱의 반대
...
## 11주차(26.05.13)
# 자바의 GUI
> GUI : 사용자가 편리하게 입출력 할 수 있도록 그래픽으로 화면을 구상하고, 마우스나 키보드로 입력 받을 수 있도록 지원하는 사용자 인터페이스
...
- 모든 GUI 컴포넌트가 상속받는 클래스 : java.awt.Componet
...
- 프레임을 닫으면 ㅍ레임에 부착된 모든 컴포넌트가 보이지 않게 됨
> 스윙 프레임 기본 구성
- 프레임 : 스윙 프로그램 기본 틀
- 메뉴바 : 메뉴들이 부착되는 공간
- 컨텐트펜 : GUI 컴포넌트들이 부착되는 공간

# 푸레임 만들기, JFrame 클래스 상속
> 스윙 프레임
...
> 이번트가 살생되면 이벤트를 처리하는 루틴(이벤트 리스너) 실행
...
- 이벤트 종류
> 사용자의 입력: 마수으 드래그, 마우스 클릭, 키보드 누름 등
> 센서로부터의 립력, 네트워크로부터 데이터 송수신
...
# 이벤트 리스너 작성 방법
> 독립 클래스로 작성
- 이벤트 리스너를 완전한 글래스로 작성
- 이벤트 리스너를 여러 곳에서 사용할 때 적합

> 내부 클래스(inner dlass)로 작성
- 클래스 안에 멤버처럼 클래스 작성
- 이벤트 리스너를 특정 클래스에서만 사용할 때 적함
